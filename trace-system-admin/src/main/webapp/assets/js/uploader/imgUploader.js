jQuery.fn.imgUploader = function(options) {
	...
	if(input.val().length > 0){
		$.each( input.val().split(","), function(i, n){
			uploadBtn.before('<li style="position:relative"><button class="close" style="position:absolute;right:8px;">×</button><img class="loaded-img" height="140px" src="' + n + '"/></li>')
		});
	}
    ...
    var uploader = Qiniu.uploader({
    	...
    	'init':{
    		'FileUploaded':function(up, file, info) {
    			...
    			uploadBtn.before('<li style="position:relative"><button type="button" class="close" style="position:absolute;right:8px;">&times;</button><img class="loaded-img"  height="140px" src="' + url + '"/></li>');
